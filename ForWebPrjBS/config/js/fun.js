...
function userInfoFormSubmitListener(form) {
  form.addEventListener('submit', function(event) {
    ...
    .then(data => {
      console.log("writeData:", data);
      if (data !== undefined && data > 0) { // 수정된 행이 있는 경우
        //.success
        alert("정보가 수정되었습니다.");
...
//codeDetail
const CodeDetailObserver = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    if (mutation.type === 'childList') {
      const boardDetailFage = document.getElementById('boardDetailFage');
      if (boardDetailFage && !boardDetailFage.dataset.listenerAdded) {
          // 리스너 추가 플래그 설정
          CodeDetailObserver.disconnect();
          fetchAllComments();
// Get the path to the file dynamically
const code_File=document.getElementById("fileIn").textContent
// Get the path to the file dynamically
...
fetch('/board_code/getAllComments') // Modify the URL if needed
.then(response => response.json())
.then(comments => {
...
CodeDetailObserver.observe(document.body, {  // container를 document.body로 변경하였습니다. 실제 container에 맞게 조정하세요.
  childList: true,
  subtree: true
});
...
function board_code(){
  ...
  .then(html =>{
    // console.log("infoooo", html)
   document.getElementById("dashboard").innerHTML=html;
   CodeDetailObserver.observe(document.body, { childList: true, subtree: true }); // Observer 재개
   executeScripts(dashboard);
  })
...
function insertComment() {
  ...
  // 서버에 댓글 데이터 전송
  fetch('/board_code/codeBoard_comments', {
  method: 'POST',
  headers: {
